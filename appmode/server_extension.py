...
class Appmode(NotebookApp):
    ...
    trusted_path = Unicode(
        "", help="Run only notebooks below this path in Appmode.", config=True
    )
    show_edit_button = Bool(
        True, help="Show Edit App button during Appmode.", config=True
    )
    show_other_buttons = Bool(
        True, help="Show other buttons, e.g. Logout, during Appmode.", config=True
    )
    temp_dir = Unicode(
        "", help="Create temporary Appmode notebooks in this directory.", config=True
    )
    ...
    def initialize_handlers(self):
        ...
        if self.trusted_path:
            self.log.info(
                "Server extension loaded with trusted path: %s", self.trusted_path
            )
        else:
...
class AppmodeHandler(ExtensionHandlerJinjaMixin, ExtensionHandlerMixin, JupyterHandler):
    ...
    async def get(self, path):
        ...
        if not path.startswith(self.trusted_path):
            self.log.warn(
                "Refused to launch %s outside trusted path %s.", path, self.trusted_path
            )
        ...
        self.add_header(
            "Cache-Control", "cache-control: private, max-age=0, no-cache, no-store"
        )
